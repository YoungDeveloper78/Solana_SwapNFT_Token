...
use mpl_token_metadata::{
    ...
    types::{ProgrammableConfig::V1, TokenStandard, TransferArgs},
};
...
pub fn send_pnft<'info>(
    ...
    builder
        ...
        .destination_token(dest_ata.key())
        .mint(nft_mint.key())
        .metadata(nft_metadata.key())
        .edition(Some(nft_edition.key()))
        .payer(*payer.key);
    ...
    if let Some(standard) = metadata.token_standard {
        if standard == TokenStandard::ProgrammableNonFungible {
            ...
            builder
                ...
                .destination_token_record(Some(dest_token_record.key()));
        ...
        match config {
            V1 { rule_set } => {
                if let Some(rule_set) = rule_set {
                    ...
                    builder.authorization_rules(Some(*rules_acc.key));
    ...
    // if let Some(vault) = from_program {
    //     invoke_signed(&transfer_ix, &account_infos, vault.as_slice())?;
    // } else {
